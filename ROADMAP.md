...
- テストは Apple の Swift Testing（`@Test` / `#expect`）で記述。140 件 (20 スイート /
...
- **DoD**:
  ...
  - 警告は preview で読める。
  - `bash scripts/verify.sh test` で SBV-U1〜U14 / SBV-I1〜I2 を含む 140 tests が緑。
...
- 現状: Swift Testing で 20 スイート / 140 テスト緑（うち 6 件 snapshot は通常 verify では skip）。
...
- **現状**: `Tests/` 配下 23 ファイル / 20 テストスイート（`struct` + `@Test`） /
  140 テスト関数。`XCTestCase` サブクラスは 0。`@testable import ShiftAlarm` を維持。
