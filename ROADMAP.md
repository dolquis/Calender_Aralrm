...
- テストは Apple の Swift Testing（`@Test` / `#expect`）で記述。172 件 (27 スイート /
  27 ファイル) 緑。通常の `scripts/verify.sh` では 6 件の snapshot test は
...
- **DoD**:
  ...
  - 警告は preview で読める。
  - `bash scripts/verify.sh test` で SBV-U1〜U14 / SBV-I1〜I2 を含む 172 tests が緑。
...
- 現状: Swift Testing で 27 スイート / 172 テスト緑（うち 6 件 snapshot は通常 verify では skip）。
...
- **現状**: `Tests/` 配下 27 ファイル / 27 テストスイート（`struct` + `@Test`） /
  172 テスト関数。`XCTestCase` サブクラスは 0。`@testable import ShiftAlarm` を維持。
