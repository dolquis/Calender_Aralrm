...
final class DayResolverTests: XCTestCase {
    ...
    func testHolidaySkipsAlarm() {
        ...
        let holiday = HolidayOverrideSnapshot(label: "元日", skipAlarm: true, replacementPresetID: nil)
        let rotation = RotationPatternSnapshot(
    ...
    func testHigherPriorityRotationWins() {
        ...
        let presets: [UUID: ShiftPresetSnapshot] = [
            ...
            highID: ShiftPresetSnapshot(id: highID, name: "High", colorHex: "#1", alarmTime: DateComponents(hour: 7, minute: 0), soundID: "s"),
        ]
        let day = date(2026, 5, 12)
        let low = RotationPatternSnapshot(id: UUID(), name: "low", anchorDate: day, cycleLength: 1, slots: [lowID], startDate: nil, endDate: nil, priority: 0, isActive: true)
        let high = RotationPatternSnapshot(id: UUID(), name: "high", anchorDate: day, cycleLength: 1, slots: [highID], startDate: nil, endDate: nil, priority: 5, isActive: true)
        let input = DayResolverInput(manualAssignments: [:], holidays: [:], rotations: [low, high], presets: presets, calendar: calendar)
        let resolved = DayResolver.resolve(date: day, input: input)
