...
public enum RotationExpander {
    ...
    public static func presetID(
        ...
        guard pattern.cycleLength > 0, pattern.slots.count == pattern.cycleLength else { return nil }
    ...
    public static func expand(
        ...
        guard pattern.cycleLength > 0, pattern.slots.count == pattern.cycleLength else { return [:] }
        var result: [Date: UUID?] = [:]
        for day in range {
            if let start = pattern.startDate, day < range.calendar.startOfDay(for: start) { continue }
