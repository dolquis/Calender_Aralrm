...
テストは Apple の Swift Testing（`@Test` / `#expect`）で記述。現状は 172 件のテスト
（27 テストスイート / 27 ファイル、うち snapshot 6 件は通常 verify で skip）を確認する。
...
古いテスト件数（現状は Swift Testing 172 件 / 27 スイート / 27 ファイル）、古いビルド
