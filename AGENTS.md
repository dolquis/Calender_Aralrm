...
テストは Apple の Swift Testing（`@Test` / `#expect`）で記述。現状は 121 件のテスト
（17 テストスイート / 20 ファイル、うち DayCell snapshot 5 件は通常 verify で skip）を確認する。
...
古いテスト件数（現状は Swift Testing 121 件 / 17 スイート / 20 ファイル）、古いビルド
